...
export function CompactExerciseCard({exercise,recommendation,selected,onOpen,onSelect}:{exercise:WorkoutExercise;recommendation?:WorkoutRecommendation;selected:boolean;onOpen:()=>void;onSelect:()=>void}){
  ...
  return <article className={`exercise-card compact-exercise-card panel${selected?' selected':''}`} onClick={onOpen}>
    ...
    {recommendation&&<div className="recommendation-reason"><strong>Оценка сочетания</strong><ul>{recommendation.reasons.slice(0,2).map(reason=><li className={`recommendation-reason-item recommendation-reason--${reason.tone}`} key={reason.id}>{reason.text}</li>)}</ul><div className="recommendation-explanation"><strong>Почему приложение так считает</strong><p>{recommendation.rule} Уверенность: {recommendation.confidence==='moderate'?'умеренная':recommendation.confidence==='limited'?'ограниченная':'тренерская логика'}.</p></div></div>}
  </article>;
}
